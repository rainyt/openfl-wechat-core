...
var Canvas = $hx_exports["Canvas"] = function() {
	...
	this._wxCanvas = ks.createCanvas();
	Canvas.CANVAS_COUNT++;
	if(window.wxdebug) {
...
HTMLImageElement.prototype = $extend(HTMLElement.prototype,{
	createImage: function() {
		...
		this._contextImage = ks.createImage();
...
HTMLUtils.isLocalAssetsNoSync = function(path,cb) {
	...
	ks.getFileSystemManager().access({ path : path, success : function(res) {
		cb(true);
	}, fail : function(res) {
		cb(false);
	}});
...
HTMLUtils.isLocalAssets = function(path) {
	...
	try {
		ks.getFileSystemManager().accessSync(path);
		return true;
	} catch( _g ) {
		return false;
	}
...
HTMLUtils.ofLocalPath = function(path) {
	var localPath = ks.env.USER_DATA_PATH + "/" + haxe_crypto_Md5.encode(path);
	return localPath;
};
HTMLUtils.cacheAssets = function(path,encoding,success,fail,callLocalPath) {
	...
	if(path.indexOf(HTMLUtils.START) == 0) {
		...
		ks.getFileSystemManager().readFile(obj);
		return true;
	}
	if(Object.prototype.hasOwnProperty.call(HTMLUtils.tempFileMaps.h,path)) {
		if(callLocalPath) {
			...
			ks.getFileSystemManager().readFile({ filePath : tmp, encoding : encoding, success : function(res) {
				success(res);
			}, fail : function(res) {
...
HTMLUtils.downloadTempFile = function(path,success,fail) {
	ks.downloadFile({ url : path, timeout : 60000, enableHttp2 : true, success : function(res) {
...
LocalStorage.prototype = {
	key: function(n) {
		var keys = ks.getStorageInfoSync().keys;
		return keys[n];
	}
	...
	,setItem: function(key,value) {
		ks.setStorage({ key : key, data : value});
		return value;
	}
...
Performance2.prototype = {
	now: function() {
		var _time = new Date().getTime() / 1000 * 1000;
		return _time;
	}
};
...
var WebSocket = $hx_exports["WebSocket"] = function(url,protocols) {
	...
	this._baseSocket = ks.connectSocket({ url : url, perMessageDeflate : false});
...
var Window = $hx_exports["Window"] = function() {
	...
	var settingDevicePixelRatio = ks.getStorageSync("wechat_devicePixelRatio");
	...
	ks.setPreferredFramesPerSecond(60);
	if(ks.onAccelerometerChange != null) {
		ks.onAccelerometerChange($bind(this,this.onAccelerometerChange));
	}
	if(ks.onAudioInterruptionBegin != null) {
		ks.onAudioInterruptionBegin($bind(this,this.onHide));
	}
	ks.onHide($bind(this,this.onHide));
	if(ks.onAudioInterruptionEnd != null) {
		ks.onAudioInterruptionEnd($bind(this,this.onShow));
	}
	ks.onShow($bind(this,this.onShow));
	if(ks.setKeepScreenOn != null) {
		ks.setKeepScreenOn({ keepScreenOn : true});
	}
	try {
		ks.onMemoryWarning(function() {
			if(ks.triggerGC != null) {
				ks.triggerGC();
			}
		});
	} catch( _g ) {
...
Window.main = function() {
	...
	Window.systemInfo = ks.getSystemInfoSync();
	Window.platform = Window.systemInfo.platform;
	Window._window = new Window();
	try {
		if(GameGlobal.isIOSHighPerformanceMode || Window.platform == "devtools" || Window.platform == "windows" || Window.platform == "mac") {
...
Window.listenerKeyEvent = function() {
	ks.onKeyDown(Window.onKeyDown);
	ks.onKeyUp(Window.onKeyUp);
};
...
Window.listenerTouchEvent = function() {
	ks.onTouchStart(Window.eventHandlerFactory("touchstart"));
	ks.onTouchMove(Window.eventHandlerFactory("touchmove"));
	ks.onTouchEnd(Window.eventHandlerFactory("touchend"));
	ks.onTouchCancel(Window.eventHandlerFactory("touchcancel"));
};
...
Window.share = function(shareData,icon,callback,extinfo,isWebPath,failCallBack) {
	...
	haxe_Log.trace("share:",{ fileName : "src/Window.hx", lineNumber : 392, className : "Window", methodName : "share", customParams : [param]});
	ks.shareAppMessage({ title : shareData, imageUrl : icon, query : param});
...
Window.prototype = $extend(HTMLElement.prototype,{
	...
	,openAccelerometerChange: function() {
		ks.startAccelerometer({ success : function() {
		}, fail : function() {
		}});
	}
	,closeAccelerometerChange: function() {
		ks.stopAccelerometer({ success : function() {
		}, fail : function() {
		}});
	}
	...
	,onShow: function(e) {
		...
		var data = window.showQuery;
		haxe_Log.trace("ks showQuery:",{ fileName : "src/Window.hx", lineNumber : 465, className : "Window", methodName : "onShow", customParams : [data]});
		if(data != null && data.from == "sidebar_new") {
			window.isSidebarCardEnter = true;
		}
		if(ks.setKeepScreenOn != null) {
			ks.setKeepScreenOn({ keepScreenOn : true});
		}
