...
var Canvas = $hx_exports["Canvas"] = function() {
	...
	this._wxCanvas = swan.createCanvas();
	Canvas.CANVAS_COUNT++;
	if(window.wxdebug) {
...
HTMLImageElement.prototype = $extend(HTMLElement.prototype,{
	createImage: function() {
		...
		this._contextImage = swan.createImage();
...
HTMLUtils.isLocalAssetsNoSync = function(path,cb) {
	...
	swan.getFileSystemManager().access({ path : path, success : function(res) {
		cb(true);
	}, fail : function(res) {
		cb(false);
	}});
...
HTMLUtils.ofLocalPath = function(path) {
	var localPath = swan.env.USER_DATA_PATH + "/" + haxe_crypto_Md5.encode(path);
	return localPath;
};
HTMLUtils.cacheAssets = function(path,encoding,success,fail,callLocalPath) {
	...
	if(path.indexOf(HTMLUtils.START) == 0) {
		...
		swan.getFileSystemManager().readFile(obj);
		return true;
	}
	return false;
};
...
HTMLUtils.downloadTempFile = function(path,success,fail) {
	swan.downloadFile({ url : path, timeout : 60000, enableHttp2 : true, success : function(res) {
...
LocalStorage.prototype = {
	key: function(n) {
		var keys = swan.getStorageInfoSync().keys;
		return keys[n];
	}
	...
	,setItem: function(key,value) {
		swan.setStorage({ key : key, data : value});
		return value;
	}
...
Performance2.prototype = {
	now: function() {
		if(Window.platform == "devtools") {
			return swan.getPerformance().now();
		}
		return swan.getPerformance().now() / 1000 | 0;
	}
};
...
var WebSocket = $hx_exports["WebSocket"] = function(url,protocols) {
	...
	this._baseSocket = swan.connectSocket({ url : url, perMessageDeflate : false});
...
var Window = $hx_exports["Window"] = function() {
	...
	var settingDevicePixelRatio = swan.getStorageSync("wechat_devicePixelRatio");
	...
	swan.setPreferredFramesPerSecond(60);
	if(swan.onAccelerometerChange != null) {
		swan.onAccelerometerChange($bind(this,this.onAccelerometerChange));
	}
	if(swan.onAudioInterruptionBegin != null) {
		swan.onAudioInterruptionBegin($bind(this,this.onHide));
	}
	swan.onHide($bind(this,this.onHide));
	if(swan.onAudioInterruptionEnd != null) {
		swan.onAudioInterruptionEnd($bind(this,this.onShow));
	}
	swan.onShow($bind(this,this.onShow));
	if(swan.setKeepScreenOn != null) {
		swan.setKeepScreenOn({ keepScreenOn : true});
	}
	try {
		swan.onMemoryWarning(function() {
			if(swan.triggerGC != null) {
				swan.triggerGC();
			}
		});
	} catch( _g ) {
...
Window.main = function() {
	...
	Window.systemInfo = swan.getSystemInfoSync();
...
Window.listenerKeyEvent = function() {
	swan.onKeyDown(Window.onKeyDown);
	swan.onKeyUp(Window.onKeyUp);
};
...
Window.listenerTouchEvent = function() {
	swan.onTouchStart(Window.eventHandlerFactory("touchstart"));
	swan.onTouchMove(Window.eventHandlerFactory("touchmove"));
	swan.onTouchEnd(Window.eventHandlerFactory("touchend"));
	swan.onTouchCancel(Window.eventHandlerFactory("touchcancel"));
};
...
Window.share = function(shareData,icon,callback,extinfo,isWebPath,failCallBack) {
	...
	swan.shareAppMessage({ title : shareData, imageUrl : icon, query : param});
...
Window.prototype = $extend(HTMLElement.prototype,{
	...
	,openAccelerometerChange: function() {
		swan.startAccelerometer({ success : function() {
		}, fail : function() {
		}});
	}
	,closeAccelerometerChange: function() {
		swan.stopAccelerometer({ success : function() {
		}, fail : function() {
		}});
	}
