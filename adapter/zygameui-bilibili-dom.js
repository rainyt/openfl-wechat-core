...
var Canvas = $hx_exports["Canvas"] = function() {
	...
	this._wxCanvas = bl.createCanvas();
	Canvas.CANVAS_COUNT++;
	if(window.wxdebug) {
...
HTMLImageElement.prototype = $extend(HTMLElement.prototype,{
	createImage: function() {
		...
		this._contextImage = bl.createImage();
		this._contextImage.onload = function() {
			...
			_gthis._contextImage.onerror = null;
		};
...
HTMLUtils.isLocalAssetsNoSync = function(path,cb) {
	...
	bl.getFileSystemManager().access({ path : path, success : function(res) {
		cb(true);
	}, fail : function(res) {
		cb(false);
	}});
...
HTMLUtils.isLocalAssets = function(path) {
	...
	try {
		bl.getFileSystemManager().accessSync(path);
		return true;
	} catch( _g ) {
		return false;
	}
...
HTMLUtils.ofLocalPath = function(path) {
	var localPath = bl.env.USER_DATA_PATH + "/" + haxe_crypto_Md5.encode(path);
	return localPath;
};
HTMLUtils.cacheAssets = function(path,encoding,success,fail,callLocalPath) {
	...
	if(path.indexOf(HTMLUtils.START) == 0) {
		...
		bl.getFileSystemManager().readFile(obj);
		return true;
	}
	return false;
};
...
HTMLUtils.downloadTempFile = function(path,success,fail) {
	bl.downloadFile({ url : path, timeout : 60000, enableHttp2 : true, success : function(res) {
...
LocalStorage.prototype = {
	key: function(n) {
		var keys = bl.getStorageInfoSync().keys;
		return keys[n];
	}
	,getItem: function(key) {
		...
		var value = bl.getStorageSync(key);
		if(value == "") {
			return null;
		} else {
			return value;
	...
	,setItem: function(key,value) {
		bl.setStorage({ key : key, data : value});
		return value;
	}
...
Performance2.prototype = {
	now: function() {
		if(Window.platform == "devtools") {
			return bl.getPerformance().now();
		}
		return bl.getPerformance().now() / 1000 | 0;
	}
};
...
var WebSocket = $hx_exports["WebSocket"] = function(url,protocols) {
	...
	this._baseSocket = bl.connectSocket({ url : url, perMessageDeflate : false});
...
var Window = $hx_exports["Window"] = function() {
	...
	var settingDevicePixelRatio = bl.getStorageSync("wechat_devicePixelRatio");
	...
	bl.setPreferredFramesPerSecond(60);
	if(bl.onAccelerometerChange != null) {
		bl.onAccelerometerChange($bind(this,this.onAccelerometerChange));
	}
	if(bl.onAudioInterruptionBegin != null) {
		bl.onAudioInterruptionBegin($bind(this,this.onHide));
	}
	bl.onHide($bind(this,this.onHide));
	if(bl.onAudioInterruptionEnd != null) {
		bl.onAudioInterruptionEnd($bind(this,this.onShow));
	}
	bl.onShow($bind(this,this.onShow));
	if(bl.setKeepScreenOn != null) {
		bl.setKeepScreenOn({ keepScreenOn : true});
	}
	try {
		bl.onMemoryWarning(function() {
			if(bl.triggerGC != null) {
				bl.triggerGC();
			}
		});
	} catch( _g ) {
...
Window.main = function() {
	...
	Window.systemInfo = bl.getSystemInfoSync();
...
Window.listenerKeyEvent = function() {
	bl.onKeyDown(Window.onKeyDown);
	bl.onKeyUp(Window.onKeyUp);
};
...
Window.listenerTouchEvent = function() {
	bl.onTouchStart(Window.eventHandlerFactory("touchstart"));
	bl.onTouchMove(Window.eventHandlerFactory("touchmove"));
	bl.onTouchEnd(Window.eventHandlerFactory("touchend"));
	bl.onTouchCancel(Window.eventHandlerFactory("touchcancel"));
};
...
Window.share = function(shareData,icon,callback,extinfo,isWebPath,failCallBack) {
	...
	bl.shareAppMessage({ title : shareData, imageUrl : icon, query : param, success : callback, fail : failCallBack == null ? null : function() {
		failCallBack();
	}});
...
Window.prototype = $extend(HTMLElement.prototype,{
	...
	,openAccelerometerChange: function() {
		bl.startAccelerometer({ success : function() {
		}, fail : function() {
		}});
	}
	,closeAccelerometerChange: function() {
		bl.stopAccelerometer({ success : function() {
		}, fail : function() {
		}});
	}
